...
#import "ShotVideoSession.h"
#import <AVFoundation/AVFoundation.h>

// 录制状态
typedef NS_ENUM( NSInteger, PKRecordingStatus ) {
    PKRecordingStatusIdle = 0,
    PKRecordingStatusStartingRecording, // 开始录制
    PKRecordingStatusRecording,
    PKRecordingStatusStoppingRecording, // 结束录制
};

@interface ShotVideoHelper() <AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate, ShortVideoSessionDelegate>
...
@property (nonatomic, strong) ShotVideoSession *assetSession;

@end
@implementation ShotVideoHelper
#pragma mark - Init
...
- (void)startRecording {
    ...
    self.assetSession = [[ShotVideoSession alloc] initWithTempFilePath:self.outputFilePath];
...
- (void)sessionDidFinishPreparing:(ShotVideoHelper *)writer {
    @synchronized(self) {
...
- (void)session:(ShotVideoHelper *)writer didFailWithError:(NSError *)error {
    @synchronized(self) {
...
- (void)sessionDidFinishRecording:(ShotVideoHelper *)writer {
    @synchronized(self) {
